import json
import sys
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QCheckBox, QDateEdit
...
def plot_data(filtered_data, start_date, end_date):
    ...
    # カラー辞書の作成
    color_dict = {
        "Nuclear": "#4B4B4B",  # チャコールグレー
        "Hydro": "#7BADE6",    # 水色
        "Thermal (Coal)": "#E97A76",  # 暗い赤
        "Thermal (Gas)": "#F2AE76",   # オレンジ
        "Thermal (Oil)": "#E97A4B",   # 赤
        "Geothermal": "#6D71E6",      # 紫
        "Wind": "#B7E686",            # 黄緑
        "Solar": "#5BA6A",            # 緑
        "Other": "#E6D174"            # 濃い黄色
    ...
    summary_df[available_formats].plot(kind='bar', stacked=True, ax=ax, width=1,
                                       color=[color_dict[fmt] for fmt in available_formats])
